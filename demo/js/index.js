...
function showImgAt(index) {
    ...
    if (!scrollbarDragging) {
        ...
        var curScrollX = Math.abs(Math.round(targetImgViewIndex * (602 / imgsCount)));
...
Draggable.create('.scroller', {
    type: 'x', bounds: { left: 0, top: 0, width: 702, height: 0 }, onDrag: function () {
        var curImgIndex = Math.abs(Math.round(this.x / (702 / imgsCount)));
...
$('.scrolller-container').on('click', function (e) {
    var curImgIndex = Math.abs(Math.round(e.offsetX / (702 / imgsCount)));
...
function resetSelected() {
    ...
    $imgs.eq(curImgViewIndex).addClass("selected");
}

$("div.container").on('mousewheel DOMMouseScroll', onMouseScroll);
function onMouseScroll(e) {
    e.preventDefault();
    var wheel = e.originalEvent.wheelDelta || -e.originalEvent.detail;
    var delta = Math.max(-1, Math.min(1, wheel));
    if (delta < 0) {//向下滚动
        showImgAt(curImgViewIndex + 1);
    } else {//向上滚动
        showImgAt(curImgViewIndex - 1);
    }
}